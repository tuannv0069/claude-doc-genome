...
this standard's own numbers — the platform sets no size limit for a rule file. They are anchored to the two it does publish, by load tier: a path-scoped rule loads on demand like a skill, so it gets the skill figure; a rule **without** `paths:` loads at launch at the same priority as AGENTS.md, carries the same adherence penalty, and gets the AGENTS.md figure. It also counts against the always-loaded total (`doc-organization.md §10`) — that total is the primary control, this per-file line is the early warning.
