<!-- generated by .agent-workspace/tooling/render_codex.py from CLAUDE.md — do not edit; edit CLAUDE.md and re-render -->
...
- `.claude/rules/conversational-output.md` — structure/tone of every conversational reply to the user (chat prose — not doc/report/code artifacts, those follow their own standard per `doc-organization.md §1`).
- `.claude/rules/critical-thinking.md` — agent decision posture — design, recommendation, approach, code review, verify/review task design.
- `.claude/rules/doc-organization.md` — organization + placement of agent doc system — rules tier, on-demand guide tree (`.agent-workspace/guide/`), skills, agents, catalogs/index files, project work-product docs.
- `.claude/rules/file-reading.md`
...
- `.claude/rules/agents-md-standards.md` — AGENTS.md — the Codex root index, loaded on every Codex turn.
- `.claude/rules/claude-md-standards.md` — CLAUDE.md (project root, loaded every turn)
- `.claude/rules/rule-writing-standards.md` — imperative verbs | cond → action | one rule one location | smallest format
- `.claude/rules/skill-md-standards.md` — SKILL.md (auto-loaded when description matches task)
- `.claude/rules/subagent-standards.md` — subagent at .claude/agents/<name>.md (auto-spawned by Claude Code orchestrator via Task tool, isolated context window)
- `.claude/rules/wiki-tier.md` — the wiki tier — claims already established about the project's subject material, written back so a later run does not derive them again. Content lives in `.agent-workspace/wiki/<subject>/<cluster>.md`, routed by `.agent-workspace/wiki/index.md`.
