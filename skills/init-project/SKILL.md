...
## dependencies
- `./portable/rules/` — 10 portable rules (always-loaded + path-scoped)
...
3b. Codex target only — render the binding surface: `python .agent-workspace/tooling/render_codex.py`. It writes `AGENTS.md` from `CLAUDE.md`, a routing stub per skill into `.agents/skills/`, and a TOML definition per agent into `.codex/agents/` (`harness-adapter.md` §5). Run it AFTER step 4, since `AGENTS.md` is rendered from the CLAUDE.md that step 4 produces; run it again after step 5 appends its triggers. Never hand-write any of those files — `render_codex.py --check` is the gate, and it is what a project wires into its pre-commit hook.
...
The Codex surface (`AGENTS.md`, `.agents/skills/*`, `.codex/agents/*`) is NOT in this map and never enters the bundle: it is generated from the live tier by `render_codex.py`, so its drift gate is `render_codex.py --check`, not a sha256 pair (`harness-adapter.md` §4).
