...
| key | value |
|---|---|
| always-loaded budget | {{ALWAYS_LOADED_BUDGET}} lines — total of `.claude/rules/*.md` without `paths:`. The three rules the genome deploys are the floor (330 lines at v2.0.0); everything above that is this project's headroom for its own always-loaded rules. Over budget → demote the least-used file to path-scoped, or split it into terse + pointer (`doc-organization.md §10`). |
