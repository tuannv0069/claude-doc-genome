...
The gate `.agent-workspace/tooling/verify_lesson_router.py` reads this table as a contract: the header row, four cells per row, `checks` targets that exist, no cycle, one row per store, and a `<critical>` block in every store declaring `scope:` and `phase:`. A freshly seeded tier — this table with no store row and no store file — passes.
