...
**CLAUDE.md < 200 lines.** This is the platform's own target, not this standard's invention: past 200 lines the file consumes more context and *adherence drops* — a long CLAUDE.md is followed less, not more. One threshold, no tiers.

Counted, not estimated: `wc -l CLAUDE.md`. Every budget here is stated in lines for that reason — an agent has no tokenizer, so a threshold in tokens is a threshold nobody can check.

over → extract per `<extract_target>` → reference. Preferred remedy is a **path-scoped rule** (`paths:` frontmatter in `.claude/rules/`), which loads only when the agent touches a matching file. Note what does NOT help: splitting into `@path` imports organizes the text but every imported file still loads at launch, so the context cost is unchanged.
...
- glossary: .agent-workspace/guide/general/glossary.md
```
...
| reference data (glossary, ID map, term def) | `.agent-workspace/guide/<area>/<topic>.md` | on-demand, no every-turn cost |
| deep how-to / patterns / examples | `.agent-workspace/guide/<area>/<topic>.md` | on-demand |
| navigation index / per-task reading map | `.agent-workspace/guide/<area>/<topic>.md` | on-demand; only read when choosing what to read next |

rule of thumb: agent needs every turn? → `rules/`. else → `.agent-workspace/guide/`.

extracted to `rules/` → must conform `rule-writing-standards.md` (xml wrap, NEVER/ALWAYS, imperative, no hedges).
extracted to `.agent-workspace/guide/` → must conform `rule-writing-standards.md` (path-scoped there); reference/nav content = `<critical>` wrap + smallest format (tables), not forced NEVER/ALWAYS.
...
✅ output: debug / root cause / RCA / "why" / "root cause" → MUST Read .agent-workspace/guide/rca-guide.md first
</example>
...
claude.md: [150 lines of test patterns]
→ extract per <extract_target> (rules/ if atomic; .agent-workspace/guide/ if how-to/data)
