...
cross-file rule placement: agent body MUST NOT inline substantive code rule (canonical shape, snippet, fix template, prop list) — reference guide §ID; see `doc-organization.md`.
</critical>
...
- inline substantive code rule in body (canonical code shape, prop list, ✅/❌ snippet, fix template) — use pointer to guide §ID per `doc-organization.md`
...
| complexity | lines (frontmatter + body) |
|------------|-----------------------------|
| narrow (single task type) | < 80 |
| general (multi-task domain) | < 250 |
| complex (multi-tool orchestration) | < 500 |

counted with `wc -l`, never estimated — an agent has no tokenizer.

this standard's own numbers — the platform states no size limit for a subagent definition. They exist because the prompt is re-sent on every spawn, so the cost recurs where a skill's does not.
...
| reference data (long lists, schemas) | extract to companion file under `.agent-workspace/guide/` |
| few-shot examples | embedded (max 3-5) |

over size budget → extract domain knowledge to skills, prompt only orchestrates
