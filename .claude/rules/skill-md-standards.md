...
cross-file rule placement: SKILL.md MUST NOT inline substantive code rule (canonical shape, snippet, fix template, prop list) — reference guide §ID; see `doc-organization.md`.
</critical>
...
Platform limits — these are the harness's own, not this standard's:

| what | limit |
|------|-------|
| SKILL.md body | **< 500 lines** |
| frontmatter `name` | ≤ 64 chars, lowercase / digits / hyphens |
| frontmatter `description` | ≤ 1024 chars |
| a referenced file | > 100 lines → open it with a table of contents |

counted with `wc -l`, never estimated — an agent has no tokenizer.

over → progressive disclosure: split into companion files referenced from SKILL.md, **one level deep** (a reference inside a reference gets partially read, so the deeper file arrives incomplete). Mutually exclusive paths belong in separate files so an unused branch costs nothing.
...
- SKILL.md body > 500 lines → split by sub-task into companion files, one level deep
