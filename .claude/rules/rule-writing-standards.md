...
|-----------|-------|
| doc | < 500 |
| doc, always-loaded (no `paths:`) | < 200 |
| rule | 1 (≤ 15 words) |
| section | < 20 |
| example | < 10 |

counted with `wc -l`, never estimated — an agent has no tokenizer, so a threshold in tokens is a threshold nobody can check.

this standard's own numbers — the platform sets no size limit for a rule file. They are anchored to the two it does publish, by load tier: a path-scoped rule loads on demand like a skill, so it gets the skill figure; a rule **without** `paths:` loads at launch at the same priority as CLAUDE.md, carries the same adherence penalty, and gets the CLAUDE.md figure. It also counts against the always-loaded total (`doc-organization.md §10`) — that total is the primary control, this per-file line is the early warning.

over → consolidate | cut hedges | extract, or add `paths:` so it loads only on matching files — which moves the file to the 500 tier as a side effect, so make the scoping honest rather than a way to buy headroom
</budget>
...
verdict: bad = 3x the size, 0.5x signal, conflates 4 rules
