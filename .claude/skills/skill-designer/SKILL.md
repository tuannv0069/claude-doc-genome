...
- `.agent-workspace/guide/general/markdown.md` — markdown formatting
- `.claude/rules/doc-organization.md` — designed skill MUST NOT inline substantive code rule; reference guide §ID instead
...
- Layout `.agent-workspace/tasks/{prefix}/sessions/{JOB_KEY}/{SESSION}/` — prefix + job + session isolation
...
- Flat layout `.agent-workspace/tasks/{prefix}/state-${SESSION}.json` (missing JOB_KEY → cross-function clutter)
