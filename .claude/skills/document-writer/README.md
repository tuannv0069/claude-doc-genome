...
Technical Editor for project documentation. Apply §A–§G writing constraints as pre-writing rules when creating or substantially rewriting a substantive `.md` document outside `.claude/**` and `.agent-workspace/guide/**`.
...
- Apply to: guidelines, specs, design docs, process docs, READMEs, business documentation outside `.claude/**` and `.agent-workspace/guide/**`.
- Skip: any path under `.claude/**` or `.agent-workspace/guide/**` (own standards), and any path the project's CLAUDE.md `## document writing` skip list declares out of scope (e.g. `detailed-design-*.md`, auto-generated).
...
- `.agent-workspace/guide/general/mermaid.md` §M7 + §2 — only when the output contains a ` ```mermaid ` block.
