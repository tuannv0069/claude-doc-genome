...
description: >
  ...
  outside `.claude/**` and `.agent-workspace/guide/**` (guideline, spec, design doc, process doc),
  ...
  (3) user explicitly invokes /document-writer on a path outside `.claude/**` and `.agent-workspace/guide/**`.
  DO NOT TRIGGER for: any path under `.claude/**` (skill files, agent instructions, rules)
  or `.agent-workspace/guide/**` (on-demand agent rule tree — governed by their own standards),
...
For each ` ```mermaid ` block in output, apply `.agent-workspace/guide/general/mermaid.md` §M7 + §2:
